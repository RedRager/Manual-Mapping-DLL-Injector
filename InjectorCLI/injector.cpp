...
bool ManualMap(HANDLE hProc, const char* szDllFile) {
	...
	data.pLoadLibaryA = LoadLibraryA;
	...
	VirtualQueryEx(hProc, pTargetBase, &memInfo, sizeof(memInfo));

	if (memInfo.State != MEM_COMMIT) {
		printf("Memory at pTargetBase is not committed.\n");
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		return false;
	}

	HANDLE hThread = CreateRemoteThread(hProc, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(Shellcode), pTargetBase, 0, nullptr);
	if (!hThread) {
...
void __stdcall Shellcode(MANUAL_MAPPING_DATA* pData)
{
	...
	auto _LoadLibaryA = pData->pLoadLibaryA;
	...
	if (pOpt->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size) {
		...
		while (pImportDescr->Name) {
			...
			HINSTANCE hDll = _LoadLibaryA(szMod);
